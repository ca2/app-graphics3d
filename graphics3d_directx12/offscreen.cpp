...
//
//
//#define VK_CHECK(x) do { HRESULT err = x; if (err) { std::cerr << "Detected DirectX12 error: " << err << std::endl; abort(); } } while (0)
//
//
...
//
//
//   offscreen_render_pass::offscreen_render_pass(::gpu::context * pgpucontextRef, VkExtent2D extent)
//      : render_pass(pgpucontextRef, extent)
//   {
//
//   }
//
//
//   offscreen_render_pass::offscreen_render_pass(::gpu::context * pgpucontextRef, VkExtent2D extent, ::pointer<render_pass> previous)
//      : render_pass(pgpucontextRef, extent, previous)
//   {
//
//   }
//
...
//
//
//   HRESULT offscreen_render_pass::acquireNextImage(uint32_t* imageIndex) 
//   {
//
//      vkWaitForFences(
...
//
//
//   HRESULT offscreen_render_pass::submitCommandBuffers(const VkCommandBuffer* buffers, uint32_t* imageIndex)
//   {
//
...
//
//      if (vkQueueSubmit(m_pgpucontext->graphicsQueue(), 1, &submitInfo, inFlightFences[currentFrame]) != VK_SUCCESS) 
//      {
//
...
//      }
//
//      VK_CHECK(vkWaitForFences(m_pgpucontext->logicalDevice(), 1, &inFlightFences[currentFrame], VK_TRUE, UINT64_MAX));
//
...
//
//
//   HRESULT offscreen_render_pass::submitSamplingWork(const VkCommandBuffer buffer, uint32_t* imageIndex)
//   {
//
//      if (imagesInFlight[*imageIndex] != VK_NULL_HANDLE)
//      {
...
//
//      ////QueueFamilyIndices indexes = m_pgpucontext->findPhysicalQueueFamilies();
...
//      VkBool32 validDepthFormat = ::graphics3d_directx12::tools::getSupportedDepthFormat(m_pgpucontext->physicalDevice, &fbDepthFormat);
...
////	// Find a suitable depth format
////	VkFormat fbDepthFormat;
////	VkBool32 validDepthFormat = ::graphics3d_directx12::tools::getSupportedDepthFormat(physicalDevice, &fbDepthFormat);
////	assert(validDepthFormat);
////
////	// Color attachment
...
////	////memAlloc.allocationSize = memReqs.size;
////	////memAlloc.memoryTypeIndex = directx12Device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
