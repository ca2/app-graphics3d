/*
 * Assorted DirectX helper functions
 *
...
// Macro to check and display DirectX return results
...
{																										\
	...
	if (res != VK_SUCCESS)																				\
	{																									\
		LOGE("Fatal : HRESULT is \" %s \" in %s at line %d"_ansi, ::graphics3d_directx::tools::errorString(res).c_str(), __FILE__, __LINE__); \
...
{																										\
	...
	if (res != VK_SUCCESS)																				\
	{																									\
		std::cout << "Fatal : HRESULT is \""_ansi << ::graphics3d_directx::tools::errorString(res) << "\" in "_ansi << __FILE__ << " at line "_ansi << __LINE__ << "\n"_ansi; \
		assert(res == VK_SUCCESS);																		\
	}																									\
}
#endif
