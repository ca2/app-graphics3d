...
namespace graphics3d_vulkan
{
   ...
   //   m_globalSetLayout = psetdescriptorlayoutGlobal;
   ...
   //void gltf_render_system::init(::gpu_vulkan::set_descriptor_layout *psetdescriptorlayoutGlobal,
   ...
   void gltf_render_system::on_prepare(::gpu::context *pgpucontext) 
   {
      ...
      m_iblLayout = ::gpu_vulkan::set_descriptor_layout::Builder(pcontext)
                       .addBinding(0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT)
                       .addBinding(1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT)
                       .addBinding(2, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT)
                       .build();
      ...
      m_pbrLayout =
         ::gpu_vulkan::set_descriptor_layout::Builder(pcontext)
      ...
      for (uint32_t i = 0; i < frameCount; i++)
      {
         ...
         //m_pdescriptorpool->allocateDescriptor(m_iblLayout->getDescriptorSetLayout(), set,
           //                                /*setIndex=*/0);
         m_pdescriptorpool->allocateDescriptor(m_iblLayout->getDescriptorSetLayout(), set);
         ...
         ::gpu_vulkan::descriptor_writer(*m_iblLayout, *m_pdescriptorpool)
            .writeImage(0, &brdfInfo)
            .writeImage(1, &irradianceInfo)
            .writeImage(2, &prefilterInfo)
            .build(set);
      ...
      {
         VkDescriptorSet set;
         //m_pdescriptorpool->allocateDescriptor(m_pbrLayout->getDescriptorSetLayout(), set, /*setIndex=*/0);
         m_pdescriptorpool->allocateDescriptor(m_pbrLayout->getDescriptorSetLayout(), set);
         ...
         ::gpu_vulkan::descriptor_writer(*m_pbrLayout, *m_pdescriptorpool)
   ...
   void gltf_render_system::createPipelineLayout(VkDescriptorSetLayout globalSetLayout)
   {
      ...
      {globalSetLayout, 
         pcontext->m_psetdescriptorlayoutGlobal->getDescriptorSetLayout(),
   ...
   void gltf_render_system::createPipeline(VkRenderPass renderPass)
   {
      ...
      auto vert = file()->as_memory(vertSpv);
      auto frag = file()->as_memory(fragSpv);
      ...
      m_opaquePipeline->initialize_graphics_pipeline(
   ...
   void gltf_render_system::on_render(::gpu::context *pgpucontext, ::graphics3d::scene *pscene)
   {
      ...
      for (auto &[id, pobject]: sceneobjects)
      {
         ...
         for (auto *node: pmodel->m_linearNodes)
         {
            ...
            vkCmdBindDescriptorSets(pcommandbuffer->m_vkcommandbuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayout, 1, 1,
                                    &node->mesh->uniformBuffer.descriptorSet, 0, nullptr);
