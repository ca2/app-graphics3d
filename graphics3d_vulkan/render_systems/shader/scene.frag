...
void main() {
    ...
    if (ALPHA_MASK && albedo.a < ALPHA_MASK_CUTOFF) {
        discard;
    }
    ...
    vec3 ambient = ubo.ambientLightColor.rgb * albedo.rgb * ubo.ambientLightColor.a;
    ...
    for (int i = 0; i < ubo.numLights; ++i) {
        ...
        vec3 diffuse  = albedo.rgb * NdotL * lightCol;
        vec3 specular = RdotV * lightCol;

        lighting += diffuse + specular;
    }

    vec3 colorEnhanced = ACESFilm(lighting * EXPOSURE);
    
    //colorEnhanced = linearToSrgb(colorEnhanced);

    outFragColor = vec4(colorEnhanced, textColor.a);
}