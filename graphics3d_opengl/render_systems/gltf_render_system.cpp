...
namespace graphics3d_opengl
{
   ...
   void gltf_render_system::on_prepare(::gpu::context *pgpucontext)
   {
      ...
      //                                    "matter://shaders/gltf_frag.frag");
      m_ppipelineOpaque->initialize_shader_with_block(pgpucontext->m_pgpurenderer, 
         as_memory_block(g_psz_pbr_vert),
                                                      as_memory_block(g_psz_pbr_frag));

      m_ppipelineMask->initialize_shader_with_block(pgpucontext->m_pgpurenderer, 
                  as_memory_block(g_psz_pbr_vert),
                                                    as_memory_block(g_psz_pbr_frag));


      m_ppipelineBlend->initialize_shader_with_block(pgpucontext->m_pgpurenderer,
         as_memory_block(g_psz_pbr_vert),
                                                     as_memory_block(g_psz_pbr_frag));


      // m_pdescriptorsetlayoutIbl = ::gpu_opengl::descriptor_set_layout::Builder(pcontext)
      //                  .addBinding(0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT)
      //                  .addBinding(1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT)
      //                  .addBinding(2, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT)
      //                  .build();
      ...
      //    ::gpu_opengl::descriptor_set_layout::Builder(pcontext)
      ...
      //
      // ::cast<::gpu_opengl::render_target> prendertarget = prenderer->m_pgpurendertarget;
      //
      // auto frameCount = prendertarget->get_frame_count();
      //
      // m_vkdescriptorsetaIbl.resize(frameCount);
      //
      // auto pdescriptorpoolbuilder = øallocate::gpu_opengl::descriptor_pool::Builder();
      //
      ...
      //
      //    ::cast<::gpu_opengl::texture> ptextureBrdf = passetmanager->m_ptextureLuBrdf;
      //    ::cast<::gpu_opengl::texture> ptextureIrrad = passetmanager->m_ptextureIrradianceCube;
      //    ::cast<::gpu_opengl::texture> ptexturePrefltr = passetmanager->m_ptexturePrefilteredCube;
      //    // auto irradianceInfo = m_assets.getIrradianceDescriptor();
      //    // auto prefilterInfo = m_assets.getPrefilteredDescriptor();
      //
      ...
      //
      //    ::gpu_opengl::descriptor_writer(*m_pdescriptorsetlayoutIbl, *m_pdescriptorpool)
      //       .writeImage(0, &brdfInfo)
      //       .writeImage(1, &irradianceInfo)
      //       .writeImage(2, &prefilterInfo)
      //       .build(set);
      ...
      //    {
      //
      //       ::cast<::gpu_opengl::texture> ptextureAlbedo = passetmanager->getTexture("cerberus_albedo");
      //       ::cast<::gpu_opengl::texture> ptextureNormal = passetmanager->getTexture("cerberus_normal");
      //       ::cast<::gpu_opengl::texture> ptextureMetallic = passetmanager->getTexture("cerberus_metallic");
      //       ::cast<::gpu_opengl::texture> ptextureRoughness = passetmanager->getTexture("cerberus_roughness");
      //       ::cast<::gpu_opengl::texture> ptextureAo = passetmanager->getTexture("cerberus_ao");
      //
      //
      ...
      //
      //       ::gpu_opengl::descriptor_writer(*m_pdescriptorsetlayoutPbr, *m_pdescriptorpool)
   ...
   //    auto descriptorsetlayout1 = ::gpu_opengl::gltf::ubo_descriptor_set_layout();
   ...
   //    opaqueConfig.attributeDescriptions = attributes;
   //
   //    m_opaquePipeline = øcreate_new<::gpu_opengl::pipeline>();
   //
   ...
   //    m_maskPipeline = øcreate_new<::gpu_opengl::pipeline>();
   //
   ...
   //    m_blendPipeline = øcreate_new<::gpu_opengl::pipeline>();
   //
   ...
   //      ::cast<::gpu_opengl::gltf::Model> pmodel = prenderable;
   ...
   //         pmodel->gltfDraw(pcommandbuffer->m_vkcommandbuffer, ::gpu_opengl::gltf::RenderFlags::RenderNone,
   //                          m_pipelineLayout, 2);
...
void gltf_render_system::on_render(::gpu::context *pgpucontext, ::graphics3d::scene_base *pscenebase)
   {
      ...
      for (auto &[id, pscenerenderable]: scenerenderables)
      {
         ...
         for (auto pmesh: pgltfmodel->m_mesha)
         {
            ...
            //glm::mat4 world = psceneobject->transform().getMatrix() * pmesh->uniformBlock.matrix;
            //glm::mat4 normalMat = glm::transpose(glm::inverse(world));
            ...
            //memcpy(node->mesh->uniformBuffer.mapped, &world, sizeof(world));
            //memcpy((char *)node->mesh->uniformBuffer.mapped + sizeof(world), &normalMat, sizeof(normalMat));
            pcommandbuffer->m_erendersystem = ::graphics3d::e_render_system_gltf_ibl;
            ...
            //    }
            //
            //    pgltfmodel->gltfDraw(pcommandbuffer->m_vkcommandbuffer, ::gpu_opengl::gltf::RenderNone, m_pipelineLayout, 2);
            //    warnedThisFrame = false;
            // }
         }
      }
