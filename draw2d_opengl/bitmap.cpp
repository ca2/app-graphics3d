...
namespace draw2d_opengl
{
   ...
   void bitmap::preserve_image(const ::i32_size & size, ::image::image * pimage)
   {
      ...
      ::gpu::bitmap::preserve_image(size, pimage);
      ...
      //auto pgputextureOld = gpu_texture(nullptr);
      ...
      //auto pgpucontextlease = pgputextureOld->acquire_context(pimage->m_pgraphicsOwned);
   ...
   void bitmap::update_bitmap_as_backed_by_gpu_texture(::gpu::texture * pgputexture, ::draw2d::graphics * pdraw2dgraphics)
   {
      ...
      m_size = pgputexture->m_textureattributes.m_sizeRaw;
      ...
      ::draw2d::graphics * pdraw2dgraphics)
   {
      ...
      if (!pimage)
      {
         ...
         throw ::exception(
            error_bad_argument,
            "OpenGL image render target requires an image");

      }

      auto sizeRaw = pimage->raw_size();

      if (!m_pgputexture || m_size != sizeRaw)
      {

         m_size = sizeRaw;
         m_iStride = sizeRaw.cx * (::i32)sizeof(::image32_t);

         ::cast < ::gpu::graphics > pgpugraphics = pdraw2dgraphics;
         ...
         if (pgpugraphics)
         {
            ...
            if (pimage->m_ppixmapOwned)
            {
               ...
               initialize_gpu_bitmap(
                  pgpugraphics->gpu_context(),
                  sizeRaw,
                  *pimage->m_ppixmapOwned);
            ...
            else
            {
               ...
               initialize_gpu_bitmap(pgpugraphics->gpu_context(), sizeRaw, {});
         ...
         else
         {
            ...
            _create_gpu_bitmap(sizeRaw, pdraw2dgraphics, pimage->m_ppixmapOwned);
   ...
   void bitmap::create_bitmap(::draw2d::graphics * pdraw2dgraphics, const ::i32_size& size, ::pixmap * ppixmap)
   {
      ...
      //   pimage32Target->vertical_swap_copy(size, m_iStride, ppixmap->image32(), iScan);
      ...
      bool bTopDown)
   {
      ...
      m_pgputexture->write_pixels(true, &pixmap, point);
...
//      ::acme::del(m_pdraw2dbitmap);
   ...
   //   pixmap.m_pimage32 = (::image32_t *) m_memOut.data();
   //   pixmap.m_pimage32Raw = (::image32_t *)m_memOut.data();
