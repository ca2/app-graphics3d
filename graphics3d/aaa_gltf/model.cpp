...
namespace gpu
{
   ...
   namespace gltf
   {
      ...
      void model::initialize_gpu_gltf_model(::gpu::context * pgpucontext,  const ::file::path & path, bool flipTexturesVertically)
      {
      ...
      void model::loadModel(const ::file::path & path, bool flipTexturesVertically)
      {
...
return pmesh;
      ...
      ::gpu::texture * model::loadMaterialTexture(aiMaterial *material, aiTextureType type)
      {
         ...
         auto iterator = m_mapTexture.find(::string(path.C_Str()));
         if (iterator != m_mapTexture.end())
         {
            return iterator->element2();
         }

         ::file::path pathImage = ::file::path(m_strDirectory) / path.C_Str();
         ...
         m_mapTexture.set_at(path.C_Str(), ptexture);
