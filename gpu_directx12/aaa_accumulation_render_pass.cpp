...
#define VK_CHECK(x) do { HRESULT err = x; if (err) { warning() << "Detected DirectX12 error: " << (int) err; abort(); } } while (0)
...
namespace gpu_directx12
{
  ...
  /*    for (int i = 0; i < depthImages.size(); i++)
   ...
   void accumulation_render_pass::createRenderPass()
   {
      ...
      //if(vkCreateRenderPass(m_pgpucontext->logicalDevice(), &renderPassInfo, NULL, &m_vkrendertargetview) != VK_SUCCESS)
      //{
   ...
   void accumulation_render_pass::createFramebuffers()
   {
      ...
      //       .renderPass = m_vkrendertargetview,
      //       .attachmentCount = 2,
      //       .pAttachments = attachments,
