...
namespace gpu_directx12
{
   ...
   LRESULT CALLBACK Draw2DirectX12WndProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
   {
   ...
   void approach::initialize_gpu_approach()
   {
      ...
      if (FAILED(result)) 
      {
         ...
         auto str = "Could not create DirectX12 instance : \n" + ::directx12::errorString(result);
      ...
      vks::android::loadDirectX12Functions(instance);
#endif
      ...
      //if (gpuCount == 0) {
      //   exitFatal("No device with DirectX12 support found", -1);
      //   throw ::exception(error_failed);
      //}
      //// Enumerate devices
      ...
      // Select physical device to be used for the DirectX12 example
      ...
      //            if (index > gpuCount - 1) {
      //               std::cerr << "Selected device index " << index << " is out of range, reverting to device 0 (use -listgpus to show available DirectX12 devices)" << "\n";
      //            }
      //            else {
      ...
      //            debug() << "Available DirectX12 devices" << "\n";
...
//
  ...
  //    pphysicaldevice->_initialize_physical_device(this, physicaldevice);
      ...
      //// DirectX12 device creation
      //// This is handled by a separate class that gets a logical device representation
      //// and encapsulates functions related to a device
      //m_pgpudevice = new vks::DirectX12Device(physicalDevice);

      //// Derived examples can enable extensions based on the list of supported extensions read from the physical device
      //getEnabledExtensions();

      //result = directx12Device->createLogicalDevice(enabledFeatures, enabledDeviceExtensions, deviceCreatepNextChain);
      //if (result != VK_SUCCESS) {
      ...
      //vkGetDeviceQueue(device, directx12Device->queueFamilyIndices.graphics, 0, &queue);
   ...
   //      defer_construct_newø(m_pdirect2ddraw2dconnector);
