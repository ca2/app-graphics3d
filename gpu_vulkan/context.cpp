...
namespace gpu_vulkan
{
                                   ...
                                   ::windowing::window *pwindow, const ::int_size &size)
   {
      ...
      if (m_pgpudevice->m_queuefamilyindexes.graphicsFamily >= 0)
      {
         ...
         vkGetDeviceQueue(
            this->logicalDevice(),
            m_pgpudevice->m_queuefamilyindexes.graphicsFamily,
            0,
...
#include "shader/white_to_color.frag.spv.inl"
   ...
   // {
   //
   //    VkCommandBufferAllocateInfo pcommandbufferCmd->m_vkcommandbufferAllocateInfo = vkinit::commandBufferAllocateInfo(pool, level, 1);
   //
   //    VkCommandBuffer pcommandbufferCmd->m_vkcommandbufferfer;
   //
   //    vkAllocateCommandBuffers(this->logicalDevice(), &pcommandbufferCmd->m_vkcommandbufferAllocateInfo, &pcommandbufferCmd->m_vkcommandbufferfer);
   //
   ...
   ::pointer<buffer> context::create_buffer(
      VkDeviceSize size,
      ...
      VkMemoryPropertyFlags properties)
   {
      ...
      allocInfo.memoryTypeIndex = m_pgpudevice->m_pphysicaldevice->findMemoryType(
...
#include "shader/blend2.frag.spv.inl"
      ...
      //if (!m_pgpurenderer->isFrameStarted)
      {
         ...
         pcommandbuffer->submitCommandBuffers(
            ptextureTarget,
            textureaSrc,
            {},
            {},
...
#include "shader/rectangle.frag.spv.inl"
   ...
   //}
    ...
    //   VkCommandBuffer pcommandbufferCmd->m_vkcommandbuffer = m_device.createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
    ...
    //         vkCmdBindDescriptorSets(pcommandbufferCmd->m_vkcommandbuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, prefilterPipeline.getPipelineLayout(),
    //         0,
    ...
    //         vkCmdCopyImage(pcommandbufferCmd->m_vkcommandbuffer, offscreen.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, prefilteredCube->m_vkimage,
       ...
       ::graphics3d::renderable * prenderableSkybox)
    {
       ...
       samplerCI.borderColor = VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE;
       VK_CHECK_RESULT(
          vkCreateSampler(this->logicalDevice(), &samplerCI, nullptr, &prefilteredCube->m_vksamplerDedicated));
       ...
       prefilterPipeline->initialize_graphics_pipeline(
          m_pgpurenderer, vert, frag, cfg);
      ...
      ::gpu::texture *environmentCubeExisting, ::graphics3d::renderable *prenderableSkybox)
   {
      ...
      pipelineconfiguration.dynamicStateInfo.dynamicStateCount = uint32_t(
         pipelineconfiguration.dynamicStateEnables.size());
   ...
   ::pointer<::gpu::texture> context::generateBRDFlut()
   {
      ...
      VkPipelineColorBlendAttachmentState blendAttachmentState = vkinit::pipelineColorBlendAttachmentState(
         0xf, VK_FALSE);
      VkPipelineColorBlendStateCreateInfo colorBlendState = vkinit::pipelineColorBlendStateCreateInfo(
      ...
      VkPipelineMultisampleStateCreateInfo multisampleState = vkinit::pipelineMultisampleStateCreateInfo(
