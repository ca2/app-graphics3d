...
namespace graphics3d_hello3d
{
   ...
   //      auto pcontext = prender->m_papplication;

   //      if (::is_set(pcontext))
   //      {

   //         pcontext->m_matrixProjection = (const ::draw3d::matrix &)matrixProjection;

   //         pcontext->m_matrixImpact = (const ::draw3d::matrix &)matrixImpact;
   ...
   ::pointer <render> impact::get_render()
   {
      ...
      prender->m_pgpucontext = system()->get_gpu()->create_context(this);

      prender->m_pgpucontext->create_offscreen_buffer(prender->m_rectangle.size());
   ...
   void impact::_001OnDraw(::draw2d::graphics_pointer & pgraphics)
   {
      ...
      auto prender = get_render();

      prender->m_strRender = m_strRender;



      prender->draw();

      prender->to_draw2d_graphics(pgraphics);
